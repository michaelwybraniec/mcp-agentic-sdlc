import { BacklogSnapshot, TaskColumn } from './types.js';
...
export function resolveTaskColumn(folder: string, status: string): TaskColumn {
...
export function validateTaskBoard(snapshot: Pick<BacklogSnapshot, 'tasks' | 'columns'>): BoardHealth {
  ...
  for (const t of tasks) {
    ...
    if (folder === 'planned' && t.status === 'completed') {
      warnings.push(
        `Task ${t.id}: status is Completed but file is still in tasks/planned/ — move to tasks/completed/ or set ${TASK_STATUS.pending}`
      );
    }
    if (folder === 'completed' && t.status !== 'completed') {
      warnings.push(
        `Task ${t.id}: file is in tasks/completed/ but # Status is not Completed — set \`# Status: ${TASK_STATUS.completed}\``
      );
    }
  ...
  if (inProgressIds.length > 1) {
    warnings.push(
      `Multiple In Progress tasks (${inProgressIds.join(', ')}) — only one should be \`# Status: ${TASK_STATUS.inProgress}\` at a time`
    );
  }
  ...
  if (inProgressIds.length === 0 && pendingPlanned.length > 0) {
    ...
    if (completedIds.length > 0) {
      warnings.push(
        `No task In Progress — set \`# Status: ${TASK_STATUS.inProgress}\` on the active task (e.g. task-${suggested}.md) or call backlog_sync with startTaskId`
      );
    } else {
      warnings.push(
        `No task marked In Progress — when starting task ${suggested}, set \`# Status: ${TASK_STATUS.inProgress}\` or call backlog_sync with startTaskId: "${suggested}"`
      );
    }
  }
  ...
  return {
    ...
    warnings,
    suggestedNextTaskId:
      inProgressIds.length === 0 && pendingPlanned.length > 0 ? pendingPlanned[0] : undefined,
  };
}

function folderFromSourcePath(sourcePath: string): string {
  const m = sourcePath.match(/tasks\/(planned|completed|unplanned)\//);
  return m ? m[1] : 'planned';
}

export function formatStatusLine(status: keyof typeof TASK_STATUS): string {
...
export function inferActiveTaskId(snapshot: BacklogSnapshot): string | undefined {
  ...
  const hasCompleted =
    snapshot.columns.completed.length > 0 ||
    Object.values(snapshot.tasks).some((t) => t.status === 'completed');

  if (hasCompleted) return pending[0];
...
export function applyInProgressInference(
  ...
  if (!inferredTaskId || snapshot.columns.inProgress.length > 0) return false;

  const task = snapshot.tasks[inferredTaskId];
  if (!task || task.column !== 'planned') return false;
